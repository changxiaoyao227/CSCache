...
struct MetaExtra {
    ...
    MetaExtra(const uint32_t &distance) {
        //含一个元素的vector,值是distance
        _past_distances = vector<uint32_t>(1, distance);
        for (uint8_t i = 0; i < n_edc_feature; ++i) {
            //window是2^10-2^19次方。max_hash_edc_idx = 2^16是2的16次方。
            //window[0]对应2^10-2^11次方时间窗口的一段时间。
            //理论上这边是初始化为0，但实际没有。
...
public:
    ...
    //MetaExtra *_extra = nullptr;
    //vector<uint32_t> _sample_times; 没初始化
...
//lookup函数中，传进来的是现在的seq
    void update(const uint32_t &past_timestamp) {
        ...
        assert(_distance);
        //看看extra指针呢 如果不存在,就新建一个  用到了MetaExtra的构建和update函数
        //
        if (!_extra) {
            _extra = new MetaExtra(_distance);
        } else
        // 调用一下就增加一下_past_distance_idx
...
//                n_edc_feature = stoull(it.second);
        ...
        //init  有n_edc_feature=10个edc窗口
        edc_windows = vector<uint32_t>(n_edc_feature);
        for (uint8_t i = 0; i < n_edc_feature; ++i) {
            //每个窗口 哦 是那个参数 base_edc_window=10 2^10-2^19
